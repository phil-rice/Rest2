...
class SimpleElementToEntityDom implements IElementToEntityDom {
    ...
    @Override public Result<ElementFail, ResourceDom> apply(TypeElement element, IViewDefnNameToViewName viewNamesMap) {
        ...
        Optional<BookmarkCodeAndUrlPattern> bookmarkAndUrlPattern = serverNames.bookmarkAndUrl(entityNames, bookmark, urlWithId, annotation.codeUrl());
        ...
        ActionsDom actionsDom = new ActionsDom(//TODO Move into own mini interface
                ...
                Optional.ofNullable(element.getAnnotation(CreateWithoutId.class)).map(create -> new CreateWithoutIdDom(rootUrl)),
                Optional.ofNullable(element.getAnnotation(Prototype.class)).map(prototype -> new PrototypeDom(prototype.value())),
                Optional.ofNullable(element.getAnnotation(PrototypeNoId.class)).map(dom -> new PrototypeNoIdDom(dom.prototypeId(), rootUrl)),
                pathDoms
        );
